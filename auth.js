/* ==========================================================================
   PULSEONE GATEKEEPER ROUTING CONTROLLER - PIPELINE & COORDINATE ENGINE
   ========================================================================== */
...
/* FIX: Synchronized IDs to perfectly match all Step 1 HTML form nodes */
function processProviderRegistryStream(event) {
    ...
    onboardingRegistrationData.ownerEmail = document.getElementById('doc-owner-email').value.trim();
    onboardingRegistrationData.ownerTel = document.getElementById('doc-owner-tel').value.trim();
    onboardingRegistrationData.superintendentName = document.getElementById('doc-super-name').value.trim();
    onboardingRegistrationData.superindependentTel = document.getElementById('doc-super-tel').value.trim();
    onboardingRegistrationData.receptionTel = document.getElementById('doc-reception-tel').value.trim();
    onboardingRegistrationData.ambulanceTel = document.getElementById('doc-ambulance-tel').value.trim();
    onboardingRegistrationData.clinicTitle = document.getElementById('doc-clinic-title').value.trim();
    onboardingRegistrationData.addressVector = document.getElementById('doc-address').value.trim();

    // Advance to Step 2 (Agreement Review Viewport)
...
async function generateAndSaveOfficialAgreementPDF() {
    try {
        // 1. Fetch original binary template file 
        ...
        const page1 = pages[0]; // Page 1 index 
        const page7 = pages[6]; // Page 7 index (Schedule-A Matrix & Signatures Block) 
        const page8 = pages[7]; // Page 8 index (Hospital Details Manifest Sheet) 

        const textColorBlue = rgb(15/255, 34/255, 64/255);
        const textColorBlack = rgb(30/255, 41/255, 59/255);

        /* ----------------==================================
           PAGE 1 INJECTIONS: TOP CORRIDOR CONTRACT DATA 
           --------------------------------================== */
        // Form Line: "executed on __ day of ____, 2026" 
        page1.drawText('16th', { x: 360, y: 675, size: 10, font: boldFont, color: textColorBlue });
        page1.drawText('June', { x: 412, y: 675, size: 10, font: boldFont, color: textColorBlue });

        // Form Line: "Hospital, having its registered office at ______________" 
        ...
        /* ----------------==================================
           PAGE 7 INJECTIONS: FOR HOSPITAL REGISTRATION UNDERLINES 
           --------------------------------================== */
        // Hospital Name Row 
        ...
        // Date Underlines
        page7.drawText('16-06-2026', { x: 390, y: 210, size: 10, font: regularFont, color: textColorBlack });
        page7.drawText('16-06-2026', { x: 90, y: 210, size: 10, font: regularFont, color: textColorBlack });

        // Handwritten Signature Image Layer Placement 
        if (onboardingRegistrationData.signatureImageBase64) {
            ...
            // Meticulously mapped right over the baseline of the original printed 'Signature:' text field 
        ...
        /* ----------------==================================
           PAGE 8 INJECTIONS: DETAILS SHEET GRID CELL ENTRIES 
           --------------------------------================== */
        // Aligned beautifully to populate the empty right-hand column slots next to labels 
        ...
        // Save structural binary changes
        const modifiedPdfBytes = await pdfDoc.save();

        // Output completed contract artifact down browser downloads
